    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[#2D5016] shadow-lg' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <Link href="/about" className="text-white hover:text-[#F5C518] transition">ABOUT</Link>
          <div className="relative" onMouseEnter={() => setDropdownOpen(true)} onMouseLeave={() => setDropdownOpen(false)}>
            ...
            {dropdownOpen && (
              <div className="absolute top-full left-0 bg-[#2D5016] min-w-48 shadow-xl">
                ...
                {categories.map((cat) => (
                  <Link key={cat._id} href={`/products/${cat.slug.current}`} className="block px-4 py-2 text-white hover:bg-[#4A8B1F] text-sm">
                    {cat.title}
                  </Link>
                ))}
              </div>
          ...
          <Link href="/contact" className="bg-[#C518] text-[#2D5016] px-4 py-2 font-bold hover:bg-[#D97706] transition">GET IN TOUCH</Link>
        </div>
      </div>
    </nav>
  )
